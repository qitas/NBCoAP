...
//#define DBG_INFO(X, Y...)	printf("%s %d:"X, __FUNCTION__, __LINE__, ##Y)
//#define DBG_ERROR(X, Y...)	printf("%s %d:"X, __FUNCTION__, __LINE__, ##Y)
...
{
	uint16_t Strlen = strlen((int8_t*)String);
...
uint32_t MQTT_EncodeMsg(MQTT_HeadStruct *Head, uint8_t *Payload, uint32_t PayloadLen, Buffer_Struct *TxBuf)
{
	...
	if (Head->DataLen)
	{
		...
		{
			if (!MQTT_AddUFT8String(TxBuf, (const uint8_t*)Head->String))
			{
				DBG_ERROR("!");
				return 0;
			}
...
uint32_t MQTT_SubscribeMsg(Buffer_Struct *TxBuf, Buffer_Struct *PayloadBuf, uint16_t PackID, MQTT_SubscribeStruct *Topic, uint32_t TopicNum)
{
	...
	{
		if (!MQTT_AddUFT8String(PayloadBuf, (const uint8_t*)Topic[i].Char))
		{
			DBG_ERROR("!");
			return 0;
		}
